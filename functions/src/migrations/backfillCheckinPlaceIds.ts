// ran with:
// ./node_modules/.bin/esbuild src/migrations/backfillCheckinPlaceIds.ts --bundle --minify --sourcemap --platform=node --target=node16 --outfile=build/backfill.js && node build/backfill.js ./service-account-private-key.json
...
async function main() {
  ...
  queryResult.forEach(async (placeDoc) => {
    ...
    checkins.forEach(async (checkinDoc) => {
      ...
      const placeId = checkinDoc.data().placeId
      if (placeId == null) {
        // eslint-disable-next-line no-console
